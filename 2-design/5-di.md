...
Названия функции-конструкторов будут начинаться с `->`, чтобы отличить их от просто функций.
...
При этом нужно явно связывать компонеты в функции main.
...
одна копия приложения, т.к. используется глобальное сотояние.
