...
Для оптимизации запросов, протокол транзакци поддерживает метод `get-many`,
а метод `get-one` легко выражается через него:
...
Что бы лучше понять, как это использовать, вгляните на тесты фейковой реализации этой абстракции:
...
Под эту абстракцию кроме SQL БД подходит еще и Redis с его оптимистическими блокировками
