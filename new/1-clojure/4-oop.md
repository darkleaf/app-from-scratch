...
`defmethod` - объявляет метод, для требуемого значения диспетчерезации.
...
используются обычные функции, и вы можете копировать реализации для нескольких записей.
